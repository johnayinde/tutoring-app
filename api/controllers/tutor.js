...
class tutorController {
      ...
      */
      ...
      try {
         ...
         for (let sub of subject) {
            ...
            if (!subjectExist) {
               const newSubject = new Subject({ subject: sub, category: level, tutors: newTutor._id })
   ...
   static async getAllTutor_Subject(req, res, next) {
      ...
      try {
         ...
         if (!exist) return res.status(423).send("You have not registed to take a class")

         const allSubjects = await Tutor.findOne({ name: req.user.name }).select('subject')
   ...
   static async deleteTutorById(req, res, next) {
      ...
      try {
         ...
         const updatedUser = await User.findOneAndUpdate({ _id: req.user._id }, { $pullAll: { subjects: docs.subject } }, { useFindAndModify: false, new: true });
         console.log('user sub deleted', updatedUser);


         const updatedCat = await Category.findOneAndUpdate({ category: 'student' }, { $pull: { tutors: docs._id } }, { useFindAndModify: false, new: true });
